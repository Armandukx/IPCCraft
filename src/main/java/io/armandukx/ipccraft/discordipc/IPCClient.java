 ...
 */
package io.armandukx.ipccraft.discordipc;

import io.armandukx.ipccraft.discordipc.entities.*;
import io.armandukx.ipccraft.discordipc.entities.Packet.OpCode;
import io.armandukx.ipccraft.discordipc.entities.pipe.Pipe;
            ...
            try
            {
                ...
                while((p = pipe.read()).getOp() != OpCode.CLOSE)
                {
                    ...
                    switch (event) {
                        ...
                        case ACTIVITY_JOIN -> LOGGER.debug("Reading thread received a 'join' event.");
                        case ACTIVITY_SPECTATE -> LOGGER.debug("Reading thread received a 'spectate' event.");
                        case ACTIVITY_JOIN_REQUEST -> LOGGER.debug("Reading thread received a 'join request' event.");
                        case UNKNOWN -> LOGGER.debug("Reading thread encountered an event with an unknown type: " +
                                json.getString("evt"));
                    }
                    if(listener != null && json.has("cmd") && json.getString("cmd").equals("DISPATCH"))
                    {
                        try
                        {
                            ...
                            switch (Event.of(json.getString("evt"))) {
                                case ACTIVITY_JOIN -> listener.onActivityJoin(this, data.getString("secret"));
                                case ACTIVITY_SPECTATE -> listener.onActivitySpectate(this, data.getString("secret"));
